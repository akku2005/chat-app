...
//       if (response.status === 200) {
//         const { accessToken, refreshToken, socketId } = response.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Store tokens and socket ID in session storage
        window.sessionStorage.setItem("accessToken", accessToken);
        window.sessionStorage.setItem("refreshToken", refreshToken);
        window.sessionStorage.setItem("socketId", socketId);

        // Emit login event to associate the email with the socket ID
